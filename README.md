...
- SPPM (確率的プログレッシブ photon mapping、既定で有効)。集光や、光源が奥まったシーンで単方向パストレーシングより桁違いに速い (12 シーンの等時間効率で 2.7 倍、水没した部屋では 290 倍)。集める光子は同じ面に載っていて実際に見えるものだけに絞る。光子は面光源と環境マップの両方から撒く。巨大なプリミティブがあって外接球が広がりすぎるシーンでは自動的にパストレースへ落ちる
...
- 素のパストレースは経路を 1 バウンスずつ別のディスパッチに分け、生きている経路だけを詰め直して次へ渡す (wavefront)。12 シーンの幾何平均で 1.24 倍速く、絵は変わらない
- VCM の vertex connection も使える (既定は off)。間接光が奥にあるシーンで上位 1% 除外の誤差が 7〜14 倍減る
