...
- SPPM (確率的プログレッシブ photon mapping)。集光や、光源が奥まったシーンで単方向パストレーシングより桁違いに速い。光子は面光源からのみ撒くので、環境光だけのシーンでは自動的にパストレースへ落ちる
- 交差判定は BVH (中央値分割)。9,680 三角形のシーンでも対話的に動く
- シーンは UI から切り替え。RTIOW の定番シーン、Cornell box、Veach の MIS テストシーン、トーラス結び目メッシュ、すりガラス/色ガラス、霧の光芒、水面の集光、間接照明だけの部屋、囲われた光源を同梱
