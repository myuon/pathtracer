...
function buildAjarDoorScene(): Scene {
  ...
  // (緑の壁が画面左)。戸口は画面の右半分に開けたいので、世界座標では
  // x が小さい側 (0 〜 S/2) が戸口になる
  const hingeX = S / 2;
  /** 扉の幅。戸口 (S/2 = 2.775) よりわずかに狭いだけにして、隙間を細く保つ */
  const doorW = 2.7;
  /** 面どうしを重ねる量。継ぎ目からの光漏れを防ぐ */
  const OVERLAP = 0.2;
  ...
  const quads: Quad[] = [
    ...
    // 奥の壁は左半分だけ。右半分はまるごと戸口 (床から天井まで) にする。
    // 鴨居を作らないのは、扉の上にも隙間ができて明るい横線が入り、
    // 扉ではなく窓枠のように見えてしまうため。
    // 厚みのある塊にしてあるのは、厚みのない 1 枚板だと床・天井・横壁と
    // 接する角でレイが数値誤差ですり抜けて、部屋の輪郭が光ってしまうため。
    // 上下と横にも 0.2 はみ出させて確実に重ねる
    ...box(
      [hingeX, -OVERLAP, S],
      [S + OVERLAP, S + OVERLAP, S + 0.3],
      white,
    ),

    // 扉本体。厚さ 0.08 の板を、戸口の外側の端 (x=0、赤い壁のそば) を
    // 蝶番にして回す。幅が戸口とほぼ同じなので、8 度ほど開くと蝶番から
    // 遠い自由端、つまり壁の中央だけに隙間が開く。
    // 蝶番の側は 0.2 だけはみ出させて、そこから光が漏れないようにする
    ...place(
      box([-OVERLAP, -OVERLAP, 0], [doorW, S + OVERLAP, 0.08], white),
      -8,
      [0, 0, S + 0.05],
    ),
    ...
    // 部屋に届くのは隙間を抜けたぶんだけなので、桁違いに強くしないと
    // 何も見えない絵になる
    {
      q: [0.35, 1.0, S + 1.7],
      u: [1.6, 0, 0],
      v: [0, 2.5, 0],
      material: emissive([550, 460, 320]),
    },
