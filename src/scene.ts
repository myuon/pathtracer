...
function buildAjarDoorScene(): Scene {
  ...
  const quads: Quad[] = [
    ...
    // 扉本体。厚さ 0.08 の板を戸口の内側の端 (x=hingeX) を蝶番にして回す。
    // 幅が戸口とほぼ同じなので、8 度ほど開くと蝶番から遠い自由端
    // (画面のいちばん右、赤い壁のそば) だけ隙間が広がる。
    // 蝶番の側は 0.2 だけ壁の塊に食い込ませて、そこから光が漏れないようにする
    ...place(
      box([-doorW, -OVERLAP, 0], [OVERLAP, S + OVERLAP, 0.08], white),
      8,
      [hingeX, 0, S + 0.05],
    ),
