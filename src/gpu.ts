...
/**
 * 解像度に見合った 1 フレームあたりの光子数。
 *
 * SPPM の 1 反復のコストは「画素数 x カメラ側 + 光子数 x 光子側」で、
 * 誤差は「カメラ側のばらつき / 反復数」と「光子密度のばらつき /
 * (反復数 x 光子数)」の和になる。時間予算を固定して最小化すると、
 * 最適な光子数は画素数とともに増える。
 *
 * 実測した最適値 (12 シーンのうち 2 〜 4 シーンで、効率 = relMSE x 秒):
 *    160x120     19,200 画素   2^14     (2^16 は 1.51 倍損)
 *    320x240     76,800 画素   2^14     (2^16 は 1.37 倍損)
 *    640x480    307,200 画素   2^14〜15 (2^16 は 1.20 倍損)
 *   1280x960  1,228,800 画素   2^15     (2^16 は 1.05 倍損)
 *
 * log2(最適値) を log2(画素数) に対して回帰すると傾き 1/6 で、4 点とも
 * これに乗る。1280x960 を基準に P^(1/6) で伸ばす。
 *
...
function photonsForPixels(pixels: number): number {
  ...
  return Math.max(4096, Math.min(PHOTON_COUNT, Math.round(n)));
}
/** ハッシュグリッドのセル数 */
