...
///
/// 詰めるのをやめて画素の並び順のまま穴を残す形も試したが、12 シーンの
/// 幾何平均が 1.222 倍 -> 1.117 倍と落ちる。詰めると生きた経路が前に寄り、
/// ワークグループが丸ごと空になるぶんが効いている
/// (enclosed 1.67 対 1.26、indirect 1.50 対 1.21)。
///
/// 唯一 mesh だけは詰めると 0.80 倍と負ける (詰めなければ 1.05 倍)。
/// atomicAdd の到着順で並びが乱れて局所性が壊れるためかと考えて、
/// ワークグループ内の順序を保って詰める形も書いてみたが 6.4 秒のまま
/// 変わらず (画素順なら 4.9 秒)、原因は並び順ではなく「生きた経路を
/// 1 つのワークグループに集めること」そのものだった。64 本ぶんの BVH 走査が
/// 同時に走ってキャッシュを奪い合う。三角形数のせいでもなく、
/// 88,200 三角形の water も 135,200 三角形の submerged も詰めた方が速い。
/// mesh だけが小さな体積に 9,680 三角形が密集したトーラス結び目で、
/// BVH が深く重なりも多い。1/12 のシーンの代償として受け入れる
